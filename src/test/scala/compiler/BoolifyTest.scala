...
class BoolifyTest extends FunSuite {
  ...
  test("addSI") {
    val testAdd = Affect("a", new Call2(addSI.asInstanceOf[Fundef2R[SI]], b, c) with ASTBt[SI])
    assert(evalCode(List(("b", 5), ("c", 2)), SI4, testAdd) == 7)
  }
  test("minSI") {
    val testMin = Affect("a", new Call2(minSI.asInstanceOf[Fundef2R[SI]], b, c) with ASTBt[SI])
