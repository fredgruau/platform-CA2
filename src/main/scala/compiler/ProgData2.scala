...
class ProgData2(private val instrs: List[Instr],private  val funs: iTabSymb[ProgData2], val tSymbVar: TabSymb[InfoNbit[_]],
  ...
  private def printConstraints(l: List[Instr], typeZone: String, rootConstraints: TabSymb[Constraint], notFolded: mutable.HashSet[String],rootConstraints2: TabSymb[Constraint]=null): Unit = {
    val cc = paquets(l).filter(_.head.locus.get!=V());  var n = 0
  ...
  /** Compute the Dag of instructions, where a neighbor is an input neigbor, i.e. affectation which set variables which are used,
  ...
  def foldRegister():ProgData2={
    ...
    for (i <- instrs) {if(i.isTransfer) i.align(constraints);   i.reset   } //simplicial instructions need not align
    ...
    val (sortedEdges, sortedNodes) = getSmaller[ZoneV,EdgeZ](minimalZones)
    println("Sorted "+ sortedNodes.mkString("\n")  )
    for(z<-z.nodes)     z.pick()
