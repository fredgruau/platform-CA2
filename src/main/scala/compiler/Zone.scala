...
object ZoneV {
  /**
   * @param instrs instruction in the zone
  ...
  def apply(instrs: Iterable[Instr], zones: TabSymb[ZoneV], c: TabSymb[Constraint]): ZoneV = {
    ...
    new ZoneV {
      val root = instrs.head.root //instruction representing the zone
      foldConstr = if (isTransfer) //we intersect the cycle constraints
        Constraint.intersect(instrs.flatMap(alignedConstr(_)).toList, locus)
...
object EdgeZ {
  ...
  def apply(i: Instr, i2: Instr, zones: TabSymb[ZoneV]): EdgeZ = {
    ...
    val partPerm: Option[Partition] = if (iloc == V() || i2loc == V()) None //throw new RuntimeException("no partition towards V" + i + i2 )
    ...
    new EdgeZ {
      lazy val in = zones(a(i2.root).name);
      lazy val out = zones(a(i.root).name);
      val partition = partPerm
    }
