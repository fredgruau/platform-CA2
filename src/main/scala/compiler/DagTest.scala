 ...
 */
   ...
   def testGetGreater(): Unit = { val(_, s)=Dag.getGreater( List(n3)); assertEquals(s, Set(n1,n2,n3))}
  ...
  def testGetCycle(): Unit =  assert (Dag.getCycle(n4) match {case Some(c) => println(c); true; case None => false  })
}
