...
object Grad {
   ...
   * */
  val slopeLtDef: Fundef1[(V, SI), ((T[E, V], B),  (E, B))] =  {
    val d = pL[V, SI]("dis")
    //val d=uI2SIL(d1)
    ...
    level.setName("level"); //vortex.setName("vortex")
    grad3.setName("grad");
    slopEv.setName("slopeLt")
    Fundef1("grad.slop", Cons(slopEv, level), d)
  }

  /** Calls slopeLt, and separate the two results. */
  def slopeLt(d: IntV): (BoolEv,  BoolE) = {
  ...
  val ltDef: Fundef1[(V, SI), (T[E, V], B)] =  {
    val d = pL[V, SI]("dis")
    //val d=uI2SIL(d1)
    val dopp: IntV= -d  //rajoute un bit zero automatique pour passer de UI a SI.
    val se: IntVe = send(List(d, d, d, dopp, dopp, dopp)) //we  apply an opp on distances comming from the center.
    ...
    grad3.setName("grad");
    slopEv.setName("slopeLt")
    Fundef1("grad.lt", slopEv, d)
  }

  /** Calls pure slope, result will be true on the side of the cell having greater value  */
  def lt(d: IntV)(implicit n: repr[B],l:repr[T[E,V]]): BoolEv = {
    new Call1[(V,SI),(T[E, V], B)] (ltDef, d)with BoolEv
  }
