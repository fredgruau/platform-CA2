...
class   Sextexrect() extends ConstLayer[V, B](1, "global")  with BranchNamed{
 ...
 //or more vertices reach the minimum.
  ...
  val choose=chooseMinOf(prioRand)
...
object Sextexrect{
 private def weakCmpProdtwo(arg1: UintV, arg2:UintV): (ASTLt[V, UI], ASTLt[V, B], ASTLt[V, B]) ={
 ...
 private def weakCmpProdZero(arg1: UintV, arg2:UintV): (ASTLt[V, UI], ASTLt[V, B], ASTLt[V, B]) ={
  val lt=ltUI2L(arg1,arg2)
  val eq=not(neqUI2L(arg1,arg2))
  val gt = ~lt & ~eq
  val min=condL(lt,arg1,arg2)
 ...
 /** here  need only the result, we are not interested in getting the minimum */
 ...
 def chooseMinOf(prio:UintV):BoolVe={
